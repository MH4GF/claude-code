...
11. `Human Review` へ動かす前に CI と feedback の close-out loop を回す
    ...
    - `gh pr view --json isDraft` で Draft 状態を確認し、Draft なら `gh pr ready` で Ready 化する。これで GitHub 側 `ready_for_review` event が発火し Slack の review 通知が飛ぶ。例外: 阻害 時の escape hatch に該当する真の外部 blocker の場合のみ Draft 維持可
