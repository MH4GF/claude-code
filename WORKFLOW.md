...
5. workpad 先頭に環境スタンプを 1 行の code fence で置く
   ...
   - 例: `mh4gfs-MacBook-Air:/Users/mh4gf/.symphony/workspaces/claude-code/MH-XX@7cf89d5`
...
8. PR URL を issue に Linear attachment として紐付ける (`## PR ルール` 節の auto-link 検証手順を実施)
...
10. workpad comment を最終状態へ更新する
    ...
    - PR URL は Linear attachment として置き、workpad 本文には重複させない
...
3. actionable なレビュアー comment (人間 / bot 問わず) は inline review comment 含めて全て blocking と扱う。次のいずれかが満たされるまで close しない
...
- 本 escape hatch に該当する時のみ PR を Draft のまま `Human Review` へ遷移してよい。通常経路では Step 2 の Ready 化手順を必ず実行する
...
- branch が push 済みで、PR が Linear attachment として issue に link されている。workpad link への fallback は不可
- PR が Ready 状態 (Draft でない)。`gh pr view --json isDraft` で確認する。例外: 阻害時の escape hatch に該当する真の外部 blocker の場合のみ Draft 許容
...
`{{ issue.identifier }}` を branch 名と PR body にそのまま埋め込む。Linear の GitHub linking は identifier 完全一致で動く。URL slug や title から推論した別形 (例: 余計な桁を足す等) を書かない。
...
- PR 作成後に Linear attachment の auto-link を検証する。`Closes <identifier>` を書いても発火しないケースがあり、放置すると `Human Review` でレビュアーから「PR が紐づけられていない」指摘が入る
  - `gh pr create` 直後から最大 60 秒、10 秒ごとに `mcp__linear-mh4gf__get_issue` を呼び `attachments[].url` に PR URL が含まれるか確認する
  - 60 秒待っても含まれなければ `mcp__linear-mh4gf__save_issue` の `links` パラメータで手動 attach する
  - payload は `{ url: "<PR URL>", title: "PR #<番号>: <PR タイトル>" }` の形にする。title を Linear native auto-link 形式と揃え、人間視点で区別が付かないようにする
  - 手動 attach したケースは workpad `### Notes` へ「auto-link 不発、手動 attach 済み」と timestamp 付きで残す。`harness-improvement` シグナルとして残し、不発頻度を後で追えるようにする
  - Linear attachment が attach 済みであることを `Human Review` 遷移前の必須ゲートとする (Completion bar に対応項目あり)
