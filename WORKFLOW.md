...
12. `Human Review` へ動かす直前に PR を Ready 状態へ揃える
    - `gh pr view --json isDraft` で Draft 状態を確認する
    - `isDraft` が `true` なら `gh pr ready` を実行して Ready 化する
    - Ready 化が成功したことを再度 `gh pr view --json isDraft` で確認する
    - これにより GitHub 側で `ready_for_review` event が発火し、Slack の review 通知が飛ぶ
    - 例外: 阻害 時の escape hatch に該当する真の外部 blocker の場合のみ Draft のまま遷移可。その時は workpad に Draft 維持の理由を書く
13. 上記を満たしたら `mcp__linear-mh4gf__save_issue` で `Human Review` へ動かす
...
14. `Todo` 起点で既に PR が attach されていたチケット は次を満たす
...
- PR が Ready 状態 (Draft でない)。`gh pr view --json isDraft` で確認する。例外: 阻害 時の escape hatch に該当する真の外部 blocker の場合のみ Draft 許容
