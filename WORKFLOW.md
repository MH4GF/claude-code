...
- `land` — Linear status が `Merging` になったら、`land` skill を loop で呼んで PR が merge されるまで進める。`gh pr merge` の実行は `land` skill 内 (SKILL.md 手順 9) でのみ行い、bg session が skill 外で直接呼ぶことは禁止
...
3. 対応フローへ route する
   ...
   - `Merging` — `land` skill を起動し、PR が merge されるまで loop する。`gh pr merge` の実行は `land` skill 内 (SKILL.md 手順 9) でのみ行い、skill 外から直接呼ばない
   - `Rework` — Step 4 へ進む
...
5. `Merging` 状態では `land` skill を loop で呼んで PR が merge されるまで進める
   ...
   - `gh pr merge` の実行は `land` skill 内 (SKILL.md 手順 9) に限定し、bg session が skill 外で直接呼ぶことは禁止 (auto-mode classifier 用の skill コンテキスト明示)
