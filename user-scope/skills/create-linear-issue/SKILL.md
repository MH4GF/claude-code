...
ユーザーの一行の意図を、`superpowers:brainstorming` で design 化してから Linear issue として起票する skill。issue body は brainstorming の design doc から組み立てる。`scripts/linear-issue.sh` (同梱) が Linear GraphQL を叩く。Linear MCP は使わない (workspace 越境事故の手当て、および スコープ を明示する目的)。
...
brainstorming skill の哲学に従う。「simple な issue こそ未検証の前提が無駄な実装を生む」。issue の Outcome / Why / 完了条件 を曖昧なまま起票すると、Symphony が拾った後の実装ワーカーは推測で動く羽目になる。brainstorming で意図を固めると design doc も成果物として永続化される。これが issue body の素材になる。
...
Skill ツール経由で `superpowers:brainstorming` を起動し、ユーザー の一行の意図を渡す。brainstorming は以下を実施する。

- project context の探索 (関連ファイル / 先行事例 / 既存 issue)
- clarifying questions (one at a time)
- 2-3 approaches の提示と user 選択
- design sections の提示と user 承認
- design doc の Write と コミット
- spec self-review (placeholder / 内部整合性 / スコープ / ambiguity)
- user による spec review

注意点:

- brainstorming skill のチェックリストでは「Invoke writing-plans skill」を terminal state と書いている
- 本 skill 経由のときは **writing-plans でなく Linear issue 起票へ進む**
- brainstorming 側が `writing-plans` を呼ぼうとしたら、ユーザー へ確認する
- 確認文言: 「create-linear-issue 経由なので writing-plans を スキップ し、design doc から Linear 起票へ進む」

design doc の置き場所は brainstorming skill の default (`docs/superpowers/specs/YYYY-MM-DD-<topic>-design.md`) を尊重する。works repo のように `docs/superpowers/specs/` が無い場合は、プロジェクトの慣行 (例: `agents/<project>/scraps/open/<topic>.md`) に従う。
...
近い既存 issue が出た場合、新規作成せず該当の identifier と URL を ユーザー へ報告して止まる。brainstorming の design doc はそのまま残るので、それ自体に価値がある。

### 4. description を組み立てる

brainstorming の design doc を素材に、次の節構成へ落とす。

- `## Outcome` — design doc の Architecture と Verification から抽出。最終状態と価値を箇条書きで 2〜4 点。手順は書かない。
- `## Why` — design doc の トリガー と context から。起票の動機・前例との関係。
- `## 参考` — design doc 自身のパス、参照した関連ファイル、関連 issue (`XX-NN` 形式、実在のみ)、外部 URL。意図にある外部参照を必ず含める。
- `## 完了条件` — design doc の Verification と完了条件から。観測可能な状態 (コマンド出力、ファイル存在、Linear / GitHub の見え方) を箇条書き。
- `## スコープ外` — design doc の Out of スコープ から。
...
プロジェクトの慣行に合わせ、80 文字以内、prefix なし、要点 1 文で書く。design doc の title を流用してもよい。
...
- design doc のパス (永続化された spec として残る)
...
過去、意図を曖昧に放置したまま起票した issue で、後から「もう少し スコープ を絞っておくべき」「先行事例を引いておくべき」と振り返る事故を起こした。brainstorming の小さなオーバーヘッドを払うと、Symphony に拾われた後の実装ワーカーは手戻りなく動ける。
...
### design doc と Linear issue を二重に持つ

design doc は git に乗る永続的な思考の記録。Linear issue は実行可能なタスクの記録。両者で読み手と目的が違うため、片方に集約しない。design doc は spec として「なぜ・どう設計したか」を担う。Linear issue は「何を達成すれば done となるか」を担う。issue 本文へ design doc のパスを参考リンクで残し、相互参照する。

## エッジケース

- 意図が複数 issue にまたがる時: brainstorming 内で「1 issue に統合か個別に分けるか」を AskUserQuestion で確認する。分けるときは brainstorming も issue ごとに分けて回す (1 issue 1 design doc)。
- 起票先 project が config に無い時: ユーザーに `~/.config/create-linear-issue/config.json` への追加を依頼してから起票する。
- wrapper が exit 4 (Linear API error) を返した時: stderr の GraphQL エラー本文を ユーザー へ報告して止まる。リトライしない。
- description の unslop 違反が残る時: 修正できなければ起票せず、現状の違反内容を ユーザー へ報告して止まる。design doc は既に コミット 済みなので、後日 再起票できる。
- brainstorming を スキップ したい正当な理由がある時 (= ユーザーが「brainstorming は不要、直接起票して」と明示した時): 例外的に brainstorming を踏まず、ステップ 2 以降のみ実行する。スキップ した事実を 報告に明記する。
