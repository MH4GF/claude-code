...
- 本セッションは無人実行のオーケストレーション。人間に追加対応を求めない。ただし人間がセッションを開き直して話しかけてきたら「対話モード」へ切り替える
...
2. 人間が PR をレビューする。少量の追加変更が必要なら、**人間が** ステータスを `In Progress` へ動かす。`kind: linear` は PR を Draft に戻す (`gh pr ready --undo`) と `gitAutomationStates.draft` 経由でも遷移する。Symphony が再ディスパッチし、bg セッションが既存 workpad から再開する。人間がこのセッションを開き直して直接修正を頼んできた場合は、この遷移を自分では行わない (「対話モード」を参照)
...
## 対話モード: 人間がセッションを開き直して話しかけてきた場合

`Human Review` へ動かして終了したセッションを、人間が後から開き直して相談や修正を頼んでくることがある。人間のメッセージが届いた時点で「無人実行」の前提は崩れ、以後は人間の指示が本スキルの手順より優先する。

- **ステータスを動かさない。** `Human Review` のまま作業する。`In Progress` へ戻すと Symphony が active state と判定して別の bg セッションをディスパッチし、同じブランチで並走する。PR を Draft に戻す操作 (`gh pr ready --undo`) も `gitAutomationStates.draft` 経由で同じ遷移を起こすので行わない
- 修正はこのセッションで直接行い、同じブランチへコミット・プッシュする。並列レビュースイープや完了バーの再実行は人間が求めたときだけ行う
- workpad は結果を反映する程度に留め、遷移の記述を書かない。「ユーザーへの次の手順を書かない」の制約はこのモードでは適用せず、人間へ普通に報告する
- 人間が Symphony に続きを任せたいと言ったときは、このセッションでは何も動かさず、人間が `In Progress` へ動かすのを待つ。人間が動かす前にこのセッションが終了している必要はないが、動かした後はこのセッションからコードに触らない
- 最終的な `Merging` への遷移も人間が行う
...
4. 再実行は次の停止条件で打ち切る。判断の詳細はコマンド本体の「再実行の停止条件」を一次ソースとする
   - コード変更を伴う対応をしたラウンドの後だけ、コミット + プッシュし、CI 確認をやり直してから `/parallel-review` を再実行する。プッシュバックと起票だけで終えたラウンドの後は再実行しない
   - ラウンド上限は 3。3 ラウンド目を終えた時点で、対応が必要な指摘が残っていても本スイープを完了とする。残件は理由付きで workpad の `### Notes` に一覧で残してから次へ進む
   - プッシュバックで完了にした指摘は次ラウンドで再出現しても対応対象から除外する。プッシュバック一覧はラウンドをまたいで workpad に保持し、再実行のたびにツール出力と突き合わせる
