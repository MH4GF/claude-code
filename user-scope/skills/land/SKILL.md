...
- マージ後のリモートブランチ削除はリポジトリの `delete_branch_on_merge` 設定に従う。`true` ならサーバー側の自動削除に任せ、`false` なら land 側で `--delete-branch` を付与して掃除する (判定 snippet はコマンド節)
...
# 許可 merge method と delete_branch_on_merge を判定 (優先順位 squash → マージコミット → rebase)
read -r allow_squash allow_merge allow_rebase delete_on_merge <<<"$(gh api repos/{owner}/{repo} \
...
# delete_branch_on_merge=false の repo では land 側からブランチを削除する
if [ "$delete_on_merge" = "true" ]; then delete_flag=""
else delete_flag="--delete-branch"
fi
...
if [ "$merge_flag" = "--squash" ]; then
  gh pr merge --squash $delete_flag --subject "$pr_title" --body "$pr_body"
else
  gh pr merge "$merge_flag" $delete_flag
fi
```
